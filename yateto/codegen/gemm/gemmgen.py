...
def tinytcGemmGen(arch, gd):
  ...
  alpha = toTinyTCImmediate(scalar_ty_c, alphaV) if isinstance(alphaV, (int, float)) else LocalValue(scalar_ty_c, 'alpha')
  beta = toTinyTCImmediate(scalar_ty_c, betaV) if isinstance(betaV, (int, float)) else LocalValue(scalar_ty_c, 'beta')
